# Goal: Get a real-time Linux running on a very common Raspberry Pi with Buildroot
...
First get Linux from rpi community and focus on the last stable branch supported by Xenomai, 3.8.y:
...
BUILDROOT=/home/kesaco/real-time-rpi/buildroot
LINUX=/home/kesaco/real-time-rpi/linux-rpi
XENOMAI=/home/kesaco/real-time-rpi/xenomai-2.6
...
System configuration
  |- System hostname (rpi-xeno)
Kernel (Unselect)
And finally build it:
make

The toolchain generated by buildroot is located under ${BUILDROOT}/output/host/usr/bin/ and is named 

5- Configure your kernel for a non-real-time experience
=======================================================

Buildroot compiled 3.8.13 Linux kernel do not boot on the Raspberry Pi if built without device tree option. Unfortunately, the default bcmrpi defconfig file does not select by default DTB support so even if Buildroot does, the build would fail because the device tree compiler does not exist (although its sources are present in the Linux kernel tree under scripts/dtc/). So we need to enable it manually. But to be able to configure our Kernel using the menuconfig command, we need a valid toolchain. That is why we compiled first the toolchain using Buildroot before starting a full build with the Kernel. To do this configuration:
cd $LINUX
...
Kernel
  ...
  |- Path to the local directory (absolute/path/to/linux-rpi)
  |- Kernel configuration (Using an in-tree defconfig file)
  |- Defconfig name (bcmrpi_quick_xeno)
  |- Kernel binary format (zImage)
  |- Build a Device Tree Blob DTB (Select)
       |- Device tree source (Use a device tree present in the kernel.)
       |- Device Tree Source file names (bcm2835-rpi-b)
...
My SD card is mounted automatically at /media/kesaco/boot and /media/kesaco/rootfs so adapt thoses values to your computer setup (may need root accesses):
...
The following is not fundamentally needed but will significantly speed up your boot time !
